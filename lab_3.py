...
signal_chirp = np.sin(2 * np.pi * f1 * t)  
signal_constant = 0.5 * np.sin(2 * np.pi * f2 * t) 
signal_decay = 0.3 * np.sin(2 * np.pi * f3 * t) * np.exp(-3 * t) 
...
def custom_cwt(x, freqs, bandwidth, fs):
    ...
    for i, f in enumerate(freqs):
        ...
        t_wavelet = np.linspace(-0.1, 0.1, 200)  
        ...
        cwt_matrix[i, :] = conv_result[:len(x)] 
